...
pattern_gawn = re.compile(r"광역시$")
pattern_special = re.compile(r"특별자치")

pattern_between_brace = re.compile(r"(?<=(\())[^\)]*(?=(\)))")

youth_preprocess_query = """ CREATE TABLE  youth_preprocess(
                            id varchar(100),
                            uri varchar(200),
                            title varchar(100),
...
youth_crawl_get_query = """
    SELECT *
    FROM youth_crawl_local
    WHERE id not in (SELECT id FROM youth_preprocess)
"""

youth_crawl_get_query_1st = """
    SELECT *
    FROM youth_crawl_local;
"""

youth_crawl_id_query = """
    SELECT id
    FROM youth_crawl_local
   
"""
youth_preprocess_date = """
...
def refine_string_to_date_time(x,flag,year=None,month=None,day=None):
    ...
    try:
        res_time = datetime.datetime(int(y),int(m),int(d))
    except ValueError:
        res_time = datetime.datetime(int(y),int(m),calendar.monthrange(int(y),int(m))[1])
        pass
...
def preprocess_local_youth():
    ...
    try:
        df = pd.read_sql(youth_crawl_get_query, con=conn)
    except SQLAlchemyError:
        df = pd.read_sql(youth_crawl_get_query_1st,con=conn)
        pass
    ...
    df['dor'] = df['title'].apply(lambda x: pattern_between_brace.search(x).group().split(' ')[0])
    df['si'] = df['title'].apply(lambda x: pattern_between_brace.search(x).group().split(' ')[1] if len(pattern_between_brace.search(x).group().split(' '))==2 else '전체' )
    
    df['si'] = df['si'].apply(lambda x: '전체' if x=='본청' else x )  
  
    df['dor'] = df['dor'].apply(lambda x: x[:-3] if pattern_gawn.search(x) else x)
     
    df['dor'] = df['dor'].apply(lambda x: x[:-5] if pattern_special.search(x) else x)
    ...
    df = df[['id','title','hash_tag','contents','school','major','job_status','specific','company','start_date','end_date','start_age','end_age','crawl_date','uri','policy_uri','dor','si']]
    ...
    #df_date = date_extract(df)
    try:
        conn.execute(youth_preprocess_query)
    ...
    try:
        conn.execute(youth_preprocess_date)
    except SQLAlchemyError as e:
        print('exist table')
        print(e)
        pass
    ...
    for i in range(len(df)):
        try:
            df_date.iloc[i:i+1].to_sql(name='youth_local_preprocess_date',if_exists='append',con=conn,index=False)
        except IntegrityError:
            pass
